...
class Schema {
    ...
    function version () {
        // Determine where the version number will be stored
        $store = $this->CI->config->item('schema_version_type');

        // Based on the type of version store that we're using, use the
        // appropriate method to get and return the version number
        if ($store == 'static') {
            // If we're using a static version store (aka the version number is
            // stored in the config file), then return the `schema_version`
            // config option
            return $this->CI->config->item('schema_version');
        } elseif ($store == 'key') {
            // Grab the names of the column of the keys and values of the
            // settings table
            $columns = $this->CI->config->item('schema_version_columns');

            // Try to load the version row from the specified settings table
            $version_result = $this->CI->db->get_where(
                $this->CI->config->item('schema_version_table'),
                array(
                    $columns['key'] =>
                        $this->CI->config->item('schema_version_key')
                )
            );
            ...
            if ($version_result->num_rows() > 0) {
                ...
                return (int) $version_result->row()->{$columns['value']};
            } else {
                // Return a version of 0 to indicate that there is probably
                // nothing installed
                return 0;
            }
        }
    ...
    function update ($version) {
        // Determine what type of version store we're using so that we can try
        // update the version number, if possible
        $store = $this->CI->config->item('schema_version_type');
        ...
        if ($store == 'key') {
            ...
            $this->CI->db->update(
                $this->CI->config->item('schema_version_table'),
                array($columns['value'], $version)
            );

            // Return true to indicate that we tried to make changes to the
            // schema version number
            return true;
        } else {
            // Return false to indicate that no changes were made to the schema
            // version number
            return false;
        }
    }
